...
def parse(source):
    ...
    # TODO: might be buggy
    return call.func.id, [
        elt.value if isinstance(elt, ast.Constant) else elt.id for elt in
        call.args[0].elts], \
           [arg.value if isinstance(arg, ast.Constant) else arg.id for arg in
            call.args[1:]], \
...
def generate_mappings(variables, columns, num_rows, sample_size_of_slicing=6,
                      seed=0):
    ...
    columns = [(True, column) for column in columns] + [(False, column) for
                                                        column in columns]
...
def test_mappings(method, dataframe, criterion="runnable", output_path=None):
    ...
    for index, mapping in generate_mappings(variables, columns,
                                            len(dataframe.index)):
        count_total_mappings += 1
        if not len(index):  # variables all correspond to strings.
            actual_mapping = [m[1] if m[0] else dataframe[m[1]] for m in
                              mapping]
        else:
            actual_mapping = [
                m[1] if m[0] else dataframe[m[1]][index[0]:index[1]] for m in
                mapping]
    ...
    for dataset in datasets:
        ...
        if output_path:
            test_mappings(method, df,
                          output_path="{}/{}_runnable_mappings.json".format(
...
def parse_wikidata_datavalue(datavalue, datatype: str):
    ...
    if datatype == 'wikibase-item':
        ...
        try:
            rst = {"wikidata ID": datavalue['value']["id"],
                   "wikidata entity type": "item",
                   "label": label_desc_dict[datavalue['value']["id"]][0],
