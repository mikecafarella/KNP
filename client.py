...
class Relation:
    ...
    def __init__(self, entity_id: str, property_id: str, isSubject: bool, rowVerbose: bool,
        ...
        if property_id:
            self.extend(property_id, isSubject, name, rowVerbose, colVerbose,
                        limit, time_property, time, label, subclass, showid)
    ...
    def query(self, require=None):
        ...
        for i in range(1, self.count + 1):
            if self.dic[i]["showid"]:
                self.df[self.dic[i]["name"] + '_' + self.dic[i]['property_id'] + '_basic'] = self.df[self.dic[i]
                                                                                                     ["name"] + '_' + self.dic[i]['property_id']].apply(lambda x: x.split('/')[-1])
        ...
        if type(func) == str:
            if func.startswith('F'):
                try:
                    ...
                    raise Exception(
                        "Not a valid function id, a valid function id should be 'Fn', n is an integer.")
            else:
                raise Exception(
                    "Not a valid function id, a valid function id should be 'Fn', n is an integer.")
        else:
            ...
            if isinstance(objcolumn, list):
                if len(objcolumn) > num_params:
                    if num_params == 1 or num_params == 0:
                        raise Exception(
                            "Too many arguments, the function only takes " + str(num_params) + " argument.")
                    else:
                        raise Exception(
                            "Too many arguments, the function only takes " + str(num_params) + " arguments.")
                if len(objcolumn) < num_required:
                    raise Exception(
                        "Too little arguments, the function requires at least " + str(num_required) + " arguments.")
                count = 0
                for key, value in params.items():
                    if value.default is inspect._empty and key != 'self':
                        if (not value.annotation is inspect._empty) and ((isinstance(value.annotation, WikiDataProperty) and not objcolumn[count].split('_')[-1] in value.annotation.type_list) or
                            ...
                            raise Exception(
                                "Please make sure the inputs meet the requirements of the arguments.")
                        count += 1
                self.df[name] = self.df[objcolumn].apply(
                    lambda x: func(*x), axis=1)
            else:
                if num_required > 1:
                    raise Exception(
                        "Too little arguments, the function requires at least " + str(num_required) + " arguments.")
                ...
                if isinstance(self.df[objcolumn][0], type_required) or type_required is inspect._empty:
                    ...
                    raise Exception(
                        "Please make sure the input meets the requirement of the argument.")
                    ...
                    if self.dic[i]["isSubject"]:
                        if not self.dic[i]['subclass']:
                            ...
                            if self.dic[i]['property_id'] == "P31":
                                ...
                                rdf_triple += """?subclasses wdt:P279+ ?""" + \
                                    self.dic[i]['focus'] + """. """
                    else:
                        if self.dic[i]["search"] is None:
                            rdf_triple += """OPTIONAL {"""
