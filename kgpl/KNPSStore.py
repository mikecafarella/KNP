...
store_session.expire_on_commit = False
conn = sqlite3.connect("KGPLData.db", check_same_thread=False)
c = conn.cursor()
...
class KNPSStore:
    ...
    def GetValue(self, id: str):
        ...
        try:
            fetch = store_session.query(kgpl.KGPLValue).filter(
    ...
    def bulk(self, val):  # need to "emtpy the list for the last time"
        ...
        if len(self.bulkval) > 9990000:
            print("lenth>10000")
            self.bulkval.append(val)
            bulk_processed_list = [(one_bulk.id, pickle.dumps(one_bulk.val),
                                    pickle.dumps(one_bulk.lineage),
                                    one_bulk.url, one_bulk.annotations,
                                    type(one_bulk).__name__, None) for one_bulk
                                   in self.bulkval]

            c.executemany(
                "INSERT OR IGNORE INTO KGPLValue VALUES (?,?,?,?,?,?,?)",
                bulk_processed_list)
            conn.commit()
            #
            ...
            #         print("!!!duplicate insertion: ROLLBACK!!! "
            #               "should never happen. "
            #               "need to check where is wrong.")
            # conn.commit()
            self.bulkval = []
        else:
            self.bulkval.append(val)
    ...
    def RegisterVariable(self, var):
        ...
        if not self.serverURL:
            ...
            cur_id = 1 + int(
                store_session.query(func.count(distinct(kgpl.KGPLVariable.id))).scalar())
            var.id = "V" + str(cur_id)
            var.url = self.selfurl + "/allvars/" + var.id
            store_session.add(var)
            store_session.commit()
        else:
    ...
    def GetVariable(self, varName: str):  # varName is V1, V2 ...
        ...
        if not self.serverURL:
            try:
                fetch = store_session.query(kgpl.KGPLVariable).filter(
                    kgpl.KGPLVariable.id == varName).order_by(
                    kgpl.KGPLVariable.timestamp.desc()).first()

                his = store_session.query(kgpl.KGPLVariable).filter(
                    kgpl.KGPLVariable.id == varName).order_by(
    ...
    def availVar(self):
        if not self.serverURL:
            try:
                num = store_session.query(kgpl.KGPLVariable.id).distinct().count()
                return int(num)
            except sqlalchemy.orm.exc.NoResultFound:
                print("-------------variable not found------------")
                return -1
    ...
    def availVal(self):
        if not self.serverURL:
            try:
                fetch = store_session.query(kgpl.KGPLValue)
                rst = []
