...
def read_into_list(list_k):
    ...
    if long_string[-1] == '\n':
        ...
        list_k = (list_k[-1] + infile.readline())[:-2]

def parse_into_dict_first_half(dict_k, list_k):
    dict_k = {}
    sz = len(list_k)
    for x in range(0, sz // 2):
        obj = json.loads(list_k[x][:-2])
        id=list(obj["entities"].keys())[0]
        dict_k[id]=[]
        dict_k[id].append(obj["entities"]["labels"].get("en", {}).get("value"))
        dict_k[id].append(obj["entities"]["descriptions"].get("en", {}).get("value"))
...
if long_string[-1] == '\n':
    ...
    List0 = long_string.splitlines()
    # make the last line complete and drop the new line character
    List0[-1] = (List0[-1] + infile.readline())[:-2]
...
for r in range(0, 1):
    if r % 2 == 0:
        ...
        t1 = threading.Thread(target=parse_info_dict_first_half, args=(dict0, List0))
        t2 = threading.Thread(target=parse_info_dict_second_half, args=(dict1, List0))
        t0.start()
        t1.start()
        t2.start()
        t0.join()
        ...
        t1 = threading.Thread(target=parse_info_dict_first_half, args=(dict0, List1))
        t2 = threading.Thread(target=parse_info_dict_second_half, args=(dict1, List1))
        t0.start()
        t1.start()
        t2.start()
        t0.join()
